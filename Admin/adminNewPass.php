...
$_db->query('DELETE FROM adminToken WHERE expiryDateTime < NOW()');

$id = req('id');

if (!is_exists($id, 'adminToken', 'id')) {
...
if (is_post()) {
    ...
    if (!in_array(true, $_err)) {
        ...
        $stm = $_db->prepare('
            UPDATE admin
            SET adminPassword = SHA1(?)
            WHERE adminId = (SELECT adminId FROM adminToken WHERE id = ?);

            DELETE FROM adminToken WHERE id = ?;
        ');
