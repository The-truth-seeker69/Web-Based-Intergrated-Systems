...
<body>
    ...
    if (is_post()) {
        ...
        if ($pfp) {
            ...
            if (!str_starts_with($pfp->type, '../../image/admin/uploads')) {
        ...
        if (!in_array(true, $_err)) { //Passed all form validation and ready for db insert
            ...
            if ($pfp) { //If there is an image
                $photo = save_photo($pfp, '../../image/admin/uploads');

                $stm = $_db->prepare('INSERT INTO admin (adminName, adminemail, adminpassword, adminphoneno, adminpic, adminRole) VALUES (?, ?, ?, ?, ?, ?)');
                $success = $stm->execute([$name, $email, $hashedPassword, $phone, $photo, 'Admin']);
            } else { //Register without image
                $stm = $_db->prepare('INSERT INTO admin (adminName, adminEmail, adminPassword, adminPhoneNo, adminRole) VALUES (?, ?, ?, ?, ?)');
                $success = $stm->execute([$name, $email, $hashedPassword, $phone, 'Admin']);
            }
            if ($success) {
                if ($autofill) {
                    session_start();
